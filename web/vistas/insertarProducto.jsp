                        ...
                        <tr align="center">
                            <td>
                                <%
                                    session = request.getSession(false);
                                %>
                                <s:form action="/actions/anadirProducto" validate="true" method="post" enctype="multipart/form-data">
                                    <td><strong>Titulo</strong></td><s:textfield key="titulo"></s:textfield>
                                    <td><strong>Descripcion</strong></td> <s:textfield key="descripcion" ></s:textfield>
                                    <td><strong>Precio</strong></td><s:textfield key="precio" value=""></s:textfield>
                                    <td><strong>Provincia</strong></td><s:select  name="provincia" key="provincia"  list="{'Álava','Albacete','Alicante','Almeria','Asturias','Ávila','Badajoz','Barcelona','Burgos','Cáceres','Cádiz','Cantabria','Castellón','Ceuta','Ciudad Real','Córdoba','Cuenca','Girona','Las Palmas','Granada','Guadalajara','Guipúzcoa','Huelva','Huesca','Islas Baleares','Jaén','A Coruña','La Rioja','León','Lleida','Lugo','Madrid','Málaga','Melilla','Murcia','Navarra','Ourense','Palencia','Pontevedra','Salamanca','Segovia','sevilla','Soria','Tarragona','Santa Cruz de Tenerife','Teruel','Toledo','Valencia','Vallalodid','Vizcaya','Zamora','Zaragoza'}"></s:select>
                                    <td><strong>SubCategoria</strong></td> <s:select  list="listaSubcategorias" listValue="nombre" name="idSubCategoria" listKey="idSubCategoria"/>
                                    <td><strong>Imagen</strong></td><s:file name="imagenArchivo" label="Imagen" size="40"/>
                                    
                                    <s:hidden name="idUsuario" value="%{#session.idUsuarioLogado}" />"
